...
class ConceptMapService {
  ...
  async processText(text, config = {}) {
    try {
      ...
      if (mergedConfig.stages.organization && mergedConfig.stages.organization.enabled) {
      ...
      if (mergedConfig.stages.reasoning && mergedConfig.stages.reasoning.enabled) {
      ...
      if (mergedConfig.stages.enrichment && mergedConfig.stages.enrichment.enabled) {
      ...
      if (mergedConfig.stages.validation && mergedConfig.stages.validation.enabled) {
      ...
      if (mergedConfig.stages.aesthetics && mergedConfig.stages.aesthetics.enabled) {
        console.log('Ejecutando Etapa 5: Estética Adaptativa');
        await this.step5_OptimizeVisualPresentation(result, mergedConfig);
        result.metadata.stageResults.aesthetics = {
          completedAt: new Date().toISOString(),
          visualStyle: mergedConfig.style,
          formatAttributesApplied: result.concepts.filter(c => c.formatting).length
        };
      }
      
      // Etapa 6: Conclusión Descriptiva
      if (mergedConfig.stages.conclusion && mergedConfig.stages.conclusion.enabled) {
      ...
      if (result.concepts.length > mergedConfig.maxConcepts) {
        console.log(`Limitando a ${mergedConfig.maxConcepts} conceptos de ${result.concepts.length} totales`);
        // Ordenar por importancia y tomar solo los primeros N
        result.concepts.sort((a, b) => b.importance - a.importance);
        const removedCount = result.concepts.length - mergedConfig.maxConcepts;
        result.concepts = result.concepts.slice(0, mergedConfig.maxConcepts);
      ...
      if (!result.concepts || result.concepts.length === 0) {
        ...
        const { jsonFormat, markdownFormat } = this.generateEducationalConceptMap(result, mergedConfig);
  ...
  _generateXMLRepresentation(result) {
    ...
    // Metadatos
    xml += '  <metadata>\n';
    xml += `    <title>${this._escapeXml(this._identifyMainTopic(result))}</title>\n`;
    xml += `    <generatedAt>${new Date().toISOString()}</generatedAt>\n`;
    xml += `    <conceptCount>${result.concepts.length}</conceptCount>\n`;
    xml += `    <relationshipCount>${result.relationships.length}</relationshipCount>\n`;
    xml += `    <coherenceScore>${result.metadata.coherenceScore || 0}</coherenceScore>\n`;
    xml += '  </metadata>\n';
    ...
    for (const concept of result.concepts) {
      xml += '    <concept>\n';
      xml += `      <id>${concept.id}</id>\n`;
      xml += `      <name>${this._escapeXml(concept.name)}</name>\n`;
      
      if (concept.definition) {
        xml += `      <definition>${this._escapeXml(concept.definition)}</definition>\n`;
      }
      
      xml += `      <importance>${concept.importance || 0}</importance>\n`;
      xml += `      <hierarchyLevel>${concept.hierarchyLevel || 0}</hierarchyLevel>\n`;
      ...
      if (concept.examples && concept.examples.length > 0) {
        ...
        for (const example of concept.examples) {
          xml += `        <example>${this._escapeXml(example)}</example>\n`;
        }
        xml += '      </examples>\n';
      }
    ...
    for (const rel of result.relationships) {
      xml += '    <relationship>\n';
      xml += `      <source>${rel.source}</source>\n`;
      xml += `      <target>${rel.target}</target>\n`;
      xml += `      <type>${this._escapeXml(rel.type || "related")}</type>\n`;
      
      if (rel.description) {
        xml += `      <description>${this._escapeXml(rel.description)}</description>\n`;
      }
    ...
    xml += '  <visualProperties>\n';
    xml += '    <layout>hierarchical</layout>\n';
    xml += `    <style>${result.metadata.configUsed?.style || "educational"}</style>\n`;
    xml += '    <colorScheme>pedagogical</colorScheme>\n';
    xml += '  </visualProperties>\n';
    
    xml += '</conceptMap>';
    
    // Asegurar que el XML usa etiquetas correctas (fix para <n> -> <name>)
  ...
  _mergeConfigs(defaultConfig, userConfig) {
    ...
    for (const key in userConfig) {
      if (userConfig.hasOwnProperty(key)) {
        // Si ambas propiedades son objetos y no arrays, combinar recursivamente
        if (
          ...
          // Si no son objetos o alguno es array, reemplazar directamente
